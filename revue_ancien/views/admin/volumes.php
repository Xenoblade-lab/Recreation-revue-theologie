...
<head>
    ...
    <title>Volumes & Numéros - Admin</title>
...
<body>
        ...
        <main class="dashboard-main">
            <div class="dashboard-header fade-up">
                <div class="header-title">
                    ...
                    <p>Gestion des volumes et numéros de la revue</p>
                </div>
                <div class="header-actions">
                    <button class="btn btn-primary" onclick="showCreateVolumeModal()">+ Créer un Volume</button>
                </div>
            </div>
            ...
            <?php if (!empty($volumes)): ?>
                <?php foreach ($volumes as $volume): ?>
                    <div class="content-card fade-up" style="margin-bottom: 1.5rem;">
                        <div class="card-header" style="display: flex; justify-content: space-between; align-items: center;">
                            <div>
                                <h2 style="margin: 0;"><?= htmlspecialchars($volume['numero_volume'] ?? 'Volume ' . $volume['annee']) ?> (<?= htmlspecialchars($volume['annee']) ?>)</h2>
                                <?php if (!empty($volume['description'])): ?>
                                    <p style="margin: 0.5rem 0 0 0; color: var(--color-gray-600); font-size: 0.9rem;"><?= htmlspecialchars($volume['description']) ?></p>
                                <?php endif; ?>
                            </div>
                            <div style="display: flex; gap: 0.5rem;">
                                <button class="btn btn-outline btn-sm" onclick="showCreateIssueModal(<?= $volume['id'] ?>)">+ Ajouter un Numéro</button>
                                <button class="btn btn-outline btn-sm" onclick="viewVolumeDetails(<?= $volume['id'] ?>)">Voir détails</button>
                                <button class="btn btn-outline btn-sm" onclick="editVolume(<?= $volume['id'] ?>)">Modifier</button>
                            </div>
                        </div>
                        
                        <?php if (!empty($volume['issues'])): ?>
                        ...
                        <?php else: ?>
                            <p style="padding: 1rem; color: var(--color-gray-600); text-align: center;">Aucun numéro dans ce volume</p>
                        <?php endif; ?>
                    </div>
                <?php endforeach; ?>
            <?php else: ?>
                <div class="content-card fade-up">
                    <p style="text-align: center; padding: 2rem; color: var(--color-gray-600);">Aucun volume créé. Créez votre premier volume pour commencer.</p>
                </div>
            <?php endif; ?>

            <!-- Numéros non assignés -->
            <?php if (!empty($unassignedIssues)): ?>
                <div class="content-card fade-up" style="margin-top: 2rem;">
                    <div class="card-header">
                        <h2>Numéros non assignés à un volume</h2>
                    </div>
                    <table class="data-table">
                        ...
                        <tbody>
                            <?php foreach ($unassignedIssues as $issue): ?>
                                <tr>
                                    <td><?= htmlspecialchars($issue['numero'] ?? '—') ?></td>
                                    <td><?= htmlspecialchars($issue['titre'] ?? '—') ?></td>
                                    <td><?= !empty($issue['date_publication']) ? htmlspecialchars($issue['date_publication']) : '—' ?></td>
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            <button class="action-btn edit" title="Assigner à un volume" onclick="assignIssueToVolume(<?= $issue['id'] ?>)">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="16" height="16">
    ...
    <!-- Modal Créer Volume -->
    <div id="createVolumeModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center; overflow-y: auto; padding: 1rem;">
        <div style="background: white; padding: 1.25rem; border-radius: 8px; max-width: 450px; width: 100%; max-height: 90vh; overflow-y: auto; margin: auto;">
            <h3 style="margin-top: 0; margin-bottom: 1rem; font-size: 1.1rem;">Créer un Volume</h3>
            <form id="createVolumeForm">
                <div class="form-field" style="margin-bottom: 0.875rem;">
                    <label style="font-size: 0.875rem; margin-bottom: 0.375rem;">Année *</label>
                    <input type="number" name="annee" required min="1980" max="2100" value="<?= date('Y') ?>" style="padding: 0.5rem; font-size: 0.875rem;">
                </div>
                <div class="form-field" style="margin-bottom: 0.875rem;">
                    <label style="font-size: 0.875rem; margin-bottom: 0.375rem;">Numéro du Volume</label>
                    <input type="text" name="numero_volume" placeholder="Ex: Volume 28" style="padding: 0.5rem; font-size: 0.875rem;">
                </div>
                <div class="form-field" style="margin-bottom: 0.875rem;">
                ...
                <div style="display: flex; gap: 0.75rem; justify-content: flex-end; margin-top: 1rem;">
                    <button type="button" class="btn btn-outline" onclick="closeCreateVolumeModal()" style="padding: 0.5rem 1rem; font-size: 0.875rem;">Annuler</button>
    ...
    <div id="createIssueModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center; overflow-y: auto; padding: 1rem;">
        <div style="background: white; padding: 1.25rem; border-radius: 8px; max-width: 450px; width: 100%; max-height: 90vh; overflow-y: auto; margin: auto;">
            ...
            <form id="createIssueForm">
                <input type="hidden" name="volume_id" id="createIssueVolumeId">
                <div class="form-field" style="margin-bottom: 0.875rem;">
    ...
    <div id="editVolumeModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center; overflow-y: auto; padding: 1rem;">
        <div style="background: white; padding: 1.25rem; border-radius: 8px; max-width: 450px; width: 100%; max-height: 90vh; overflow-y: auto; margin: auto;">
            ...
            <form id="editVolumeForm">
                <input type="hidden" name="volume_id" id="editVolumeId">
                <div class="form-field" style="margin-bottom: 0.875rem;">
                    <label style="font-size: 0.875rem; margin-bottom: 0.375rem;">Numéro du Volume</label>
                    <input type="text" name="numero_volume" id="editVolumeNumero" placeholder="Ex: Volume 28" style="padding: 0.5rem; font-size: 0.875rem;">
                </div>
                <div class="form-field" style="margin-bottom: 0.875rem;">
                    <label style="font-size: 0.875rem; margin-bottom: 0.375rem;">Description</label>
                    <textarea name="description" id="editVolumeDescription" rows="2" style="padding: 0.5rem; font-size: 0.875rem;"></textarea>
                </div>
                <div style="display: flex; gap: 0.75rem; justify-content: flex-end; margin-top: 1rem;">
    ...
    <!-- Modal Assigner Numéro à Volume -->
    <div id="assignIssueModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 1000; align-items: center; justify-content: center; overflow-y: auto; padding: 1rem;">
        <div style="background: white; padding: 1.25rem; border-radius: 8px; max-width: 450px; width: 100%; max-height: 90vh; overflow-y: auto; margin: auto;">
            <h3 style="margin-top: 0; margin-bottom: 1rem; font-size: 1.1rem;">Assigner le Numéro à un Volume</h3>
            <form id="assignIssueForm">
                <input type="hidden" name="issue_id" id="assignIssueId">
                <div class="form-field" style="margin-bottom: 0.875rem;">
                    <label style="font-size: 0.875rem; margin-bottom: 0.375rem;">Volume *</label>
                    <select name="volume_id" id="assignIssueVolumeId" required style="padding: 0.5rem; font-size: 0.875rem;">
                        <option value="">Sélectionner un volume</option>
                        <?php foreach ($volumes as $vol): ?>
                            <option value="<?= $vol['id'] ?>"><?= htmlspecialchars($vol['numero_volume'] ?? 'Volume ' . $vol['annee']) ?> (<?= $vol['annee'] ?>)</option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div style="display: flex; gap: 0.75rem; justify-content: flex-end; margin-top: 1rem;">
                    ...
                    <button type="submit" class="btn btn-primary" style="padding: 0.5rem 1rem; font-size: 0.875rem;">Assigner</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        function showCreateVolumeModal() {
            document.getElementById('createVolumeModal').style.display = 'flex';
        }
        ...
        document.getElementById('createVolumeForm').addEventListener('submit', function(e) {
            e.preventDefault();
            const formData = new FormData(this);
            
            fetch('<?= Router\Router::route("admin") ?>/volumes/create', {
                method: 'POST',
                body: formData
            })
            ...
            .then(data => {
                if (data.success) {
                    showToast('Volume créé avec succès !', 'success');
        ...
        function viewVolumeDetails(id) {
            window.location.href = '<?= Router\Router::route('admin') ?>/volume/' + id;
        }

        function editVolume(id) {
            // Récupérer les données du volume depuis le DOM
            const volumeCard = event.target.closest('.content-card');
            const volumeTitle = volumeCard.querySelector('h2').textContent;
            const volumeDescription = volumeCard.querySelector('p')?.textContent || '';
            
            document.getElementById('editVolumeId').value = id;
            document.getElementById('editVolumeNumero').value = volumeTitle.split('(')[0].trim();
            document.getElementById('editVolumeDescription').value = volumeDescription;
        ...
        function editIssue(id) {
            // Récupérer les données du numéro depuis la ligne du tableau
        ...
        function assignIssueToVolume(issueId) {
            document.getElementById('assignIssueId').value = issueId;
            document.getElementById('assignIssueVolumeId').value = '';
        ...
        // Formulaire assigner numéro à volume
        document.getElementById('assignIssueForm').addEventListener('submit', function(e) {
            e.preventDefault();
            const formData = new FormData(this);
            
            fetch('<?= Router\Router::route("admin") ?>/issues/assign-volume', {
                method: 'POST',
                body: formData
            })
            .then(response => response.json())
            .then(data => {
                if (data.success) {
                    showToast('Numéro assigné au volume avec succès !', 'success');
                    setTimeout(() => window.location.reload(), 1500);
                } else {
                    showToast('Erreur : ' + (data.error || 'Une erreur est survenue'), 'error');
                }
