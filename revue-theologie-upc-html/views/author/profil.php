...
<div class="dashboard-header">
  <h1><?= htmlspecialchars(__('author.profil_title')) ?></h1>
  <p><?= htmlspecialchars(__('author.profil_intro')) ?></p>
</div>
<?php if ($error): ?>
<p class="text-accent mb-4"><?= htmlspecialchars($error) ?></p>
<?php endif; ?>
<?php if ($success): ?>
<p class="mb-4" style="color: var(--primary);"><?= htmlspecialchars(__('author.profil_success')) ?></p>
<?php endif; ?>
<div class="dashboard-card">
  <form method="post" action="<?= $base ?>/author/profil" class="space-y-4">
    <?= csrf_field() ?>
    <div>
